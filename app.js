...
function fitCanvas(){
  ...
  const maxH = 420; // good for laptop; on mobile will shrink with width
...
function draw(){
  ...
  for(let pass=0; pass<2; pass++){
    ...
    for(let i=0;i<=steps;i++){
      ...
      // rotate by inclination around x, then azimuth around y
      // Rx
      ...
      if(pass===0 && z2>0) { // back half
        ctx.strokeStyle = 'rgba(111,211,255,0.25)';
      } else if(pass===1 && z2<=0){ // front half
  ...
  // draw central planet with shading
  ...
  grd.addColorStop(0, current.colors.planet);
  grd.addColorStop(1, '#0a1826');
  ctx.fillStyle = grd;
  ctx.fill();

  // satellite position advancing on orbit
  const omega = Math.sqrt(current.mu / (current.r**3)); // rad/s
  ...
  // draw satellite
  ctx.beginPath();
  ctx.fillStyle = current.colors.sat;
  ctx.arc(P.x, P.y, 6*(P.s), 0, Math.PI*2);
  ctx.fill();

  // velocity vector (tangent in 3D, projected)
...
el('casioBtn').addEventListener('click', ()=>{ setPreset('casio'); compute(); });
...
inclTxt.textContent = incl.value; azTxt.textContent = az.value; spdTxt.textContent = spd.value;
compute(); fitCanvas();
